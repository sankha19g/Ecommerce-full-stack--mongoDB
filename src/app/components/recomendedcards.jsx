...
export default async function Recomendedcards() {
            ...
            {products.map((props) => (
                ...
                <div key={props._id} className="w-full h-fit rounded-xs border-2 border-slate-200 flex flex-col justify-between cursor-pointer  hover:bg-slate-100">
                    ...
                    <div className="m-2 w-auto h-fit rounded-md p-1">
                        ...
                        <p className="text-sm truncate ">{props.title || "No Title"}</p>
                        <div className="flex flex-row items-center gap-2">
                            <p className="text-sm text-[#cc1c39] font-weight-200"> - {(((props.costprice - props.price) / props.costprice) * 100).toFixed(2)}</p>

                            <p className="text-sm font-bold">{`₹${props.price || "No Price"}`}</p>
                        ...
                        <div className="flex text-sm  flex-row justify-between gap-2 mt-2">
                            <button className="cursor-pointer w-25  bg-amber-200 hover:bg-amber-400 rounded-md  px-1 py-1 ">Add to cart</button>
                            <Link href={`/productsdetails/${props._id}`}><button className="cursor-pointer w-25 bg-amber-200 hover:bg-amber-400 rounded-md px-1 py-1 ">Buy now</button></Link>
                        </div>
                    </div>
